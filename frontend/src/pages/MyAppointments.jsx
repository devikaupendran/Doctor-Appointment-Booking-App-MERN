...
const MyAppointments = () => {
    ...
    const cancelAppointment = async (appointmentId) => {
        try {
            ...
            if (data.success) {
                ...
                getUserAppointments();//update user appointment
                getDoctorsData();
            } else {
    ...
    const initPay = (order) => {
        const options = {
            ...
            handler: async (response) => { //whenever the payment done successfully we get a response in this fn
                    ...
                    appointments.map((item, index) => {
                        return (
                            <div key={index} className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b border-b-gray-300'>
                                <div>
                                    <img src={item.doctorData.image} className='w-32 bg-indigo-50' />
                                </div>

                                <div className='flex-1 text-sm text-zinc-600'>
                                    <p className='text-neutral font-semibold'>{item.doctorData.name}</p>
                                    ...
                                    <p className='text-sm mt-1'><span className='text-sm text-neutral-700 font-medium' >Date & Time:  {slotDateFormat(item.slotDate)} |  {item.slotTime} </span> </p>
                                </div>
                                ...
                                <div className='flex flex-col gap-2 justify-end'>
                                    {
                                        ...
                                        <button className='sm:min-w py-2 border rounded text-stone-500 bg-indigo-50'>Paid</button>
                                    }
